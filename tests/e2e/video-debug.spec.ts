...
describe("Video Generation Debug", () => {
  it(
    ...
    async () => {
      ...
      console.log("Generate Video Data:", JSON.stringify(generateData, null, 2));
        ...
        try {
          ...
          if (!statusResponse.ok) {
            ...
            if (statusResponse.status === 500) {
              ...
              console.error("The video may still be processing. Try checking manually later.");
          ...
          if (statusData.status === "complete" || statusData.status === "completed" || statusData.status === "done") {
            console.log("\n✅ Video generation COMPLETE!\n");
            
            // Check for video data in various possible field names
            const videoData = statusData.video_base64 || statusData.video || statusData.data || statusData.result;
            ...
            if (videoData) {
              ...
              expect.fail("Video generation completed but no video data returned");
            }
          }
          ...
          if (statusData.status === "error" || statusData.status === "failed" || statusData.error) {
            ...
            console.error("Error:", statusData.error || statusData.message || "Unknown error");
            expect.fail(`Video generation failed: ${statusData.error || "Unknown error"}`);
          }
      ...
      if (!videoUrl) {
        ...
        console.error("\nPossible issues:");
        console.error("1. Backend video generation is taking longer than expected");
        ...
        expect.fail(`Video generation timed out after ${finalElapsedSeconds}s. Last status: ${lastStatus}`);
      }
  ...
  it(
    "should check an existing operation status (manual test)",
    async () => {
      if (!authToken) {
        console.log("Skipping - no auth token");
        return;
      ...
      if (!manualOperationName) {
        console.log("⊘ Skipping manual operation check - set MANUAL_OPERATION_NAME env var to use");
        return;
      }
