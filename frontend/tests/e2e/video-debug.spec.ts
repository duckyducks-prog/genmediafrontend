...
// Skip this test suite when using mocks since it's designed for real E2E testing
const shouldSkip = process.env.VITE_MOCK_API === 'true';

const API_BASE_URL = "https://veo-api-otfo2ctxma-uc.a.run.app";
...
describe.skipIf(shouldSkip)("Video Generation Debug", () => {
  it(
    "should generate a video and log all API responses",
    async () => {
      if (shouldSkip) {
        console.log("⏭️  Skipping video debug test - mocking is enabled");
        return;
      }
  ...
  it("should check an existing operation status (manual test)", async () => {
    if (shouldSkip) {
      console.log("⏭️  Skipping manual status test - mocking is enabled");
      return;
    }
    if (!authToken) {
      console.log("Skipping - no auth token");
